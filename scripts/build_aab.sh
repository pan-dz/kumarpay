...
usage() {
  ...
  echo "Example (from Git tag): $0 --name KumarPay-2602011330 --tag --build 45 --env prod --output-dir build/aab"
}
...
OUTPUT_DIR="${OUTPUT_DIR:-build/aab}"
mkdir -p "$OUTPUT_DIR"

new_name="${APP_NAME}.aab"
cp "$AAB_PATH" "$OUTPUT_DIR/$new_name"
